...
export class ReporteArticuloService {
  ...
  obtenerFacturas(obj){
    ...
    return this.http.post<string>(baseURL() +'/entradas-por-articulo/',obj,{headers, responseType: 'text' as 'json'});
  }
  ...
  reporteExcel(obj){
    if(obj.length === 0 || typeof obj === 'undefined'){
      ...
      let titulo = 'Reporte Venta de Productos',
          arr= [];
      const header = ["Fecha","Código", "Artículo","Categoría", "Subtotal", "Total Impuesto", "Impuesto Servicio", "Total"];
  ...
  reporteWeb(obj,filtros){
    const tipoReporte = 'Articulos'; 
    const tipoComprobante = '08'
